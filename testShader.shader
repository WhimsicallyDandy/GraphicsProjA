   ...
   SubShader { 
      Pass { 
         ...
         vertexOutput vert(vertexInput input) 
         {
			...
			// maps colour to direction/intensity of normal vector
			output.col = float4((input.normal + float3(1.0, 1.0, 1.0))/2.0, 1.0); // ooh this is nice

			// texcoords correspond to the coordinates for where textures would go, different kinds
            return output;
         }
